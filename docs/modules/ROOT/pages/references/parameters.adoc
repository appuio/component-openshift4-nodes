...
[NOTE]
====
If any of the fields in `spec.resourceLimits` are configured, the component injects the following "legacy" default values before merging:
[source,yaml]
----
spec:
  resourceLimits:
...
Make sure `autoscaling.ignoreDownscalingSync` is set to true when using the scheduled downscaling.
...
=== `autoscaling.upgradejobhooks.enabled`

[horizontal]
type:: bool
default:: `false`

Whether to enable upgradejobhooks that enable downscaling during the maintenance.
Make sure `autoscaling.ignoreDownscalingSync` is set to true when using the downscaling upgradejobhooks.

=== `autoscaling.upgradejobhooks.upgrade_job_selector`

[horizontal]
type:: dictionary
default:: `{}`

Selector for the upgradejobs that should execute the hooks.
...
== `machineConfigPruning`

[horizontal]
type:: dict
default:: See `class/defaults.yml`

This paramter allows to configure a CronJob that regularly prunes old, unused MachineConfigs, so they don't accumulate endlessly on a cluster.
...
[IMPORTANT]
The component will print a warning if the configuration field `maxPods` is set to a value larger than 250.
  ...
  groups:
    PodCapacity:
      rules:
        TooManyPods:
          annotations:
            message: 'The number of pods is too damn high' <5>
          for: 3h <6>
        ExpectTooManyPods:
          expr: <7>
            range: '2d'
    ...
    ResourceRequests:
      rules:
        TooMuchMemoryRequested:
          enabled: true
          expr:
            raw: sum(kube_pod_resource_request{resource="memory"}) > 9000*1024*1024*1024 <8>
    CpuCapacity:
      rules:
        ClusterCpuUsageHigh:
          enabled: false <9>
...
<3> List of machinesets to monitor. This is used only if machineSets are in use. If machineSets are not used, the parameter `monitorNodeRoles` is used instead.
<4> Dictionary of aggregated machine sets. The key is the name that should be given to the aggregated machine set. The value is a regular expression matching all machinesets that should be aggregated into this value.
If multiple aggregations are provided, they must be disjoint.
It is possible to use substitutions in the key, such as `'$1': (.+)-.+` - this would result in all machinesets being aggregated according to their name split at the first dash.
<5> Changes the alert message for the pod capacity alert
<6> Only alerts for pod capacity if it fires for 3 hours
<7> Change the pod count prediction to look at the last two days and predict the value in five days
<8> Completely overrides the default alert rule and alerts if the total memory request is over 9000 GB
<9> Disables both CPU capacity alert rules
<10> Disables alert if the cluster has unused nodes.
